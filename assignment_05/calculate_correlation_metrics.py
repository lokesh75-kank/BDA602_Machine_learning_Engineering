...
def create_corrheatmapfigs(corr_cat_cat, corr_con_cat, corr_con_con):
    ...
    # reshape the dataframe to a matrix using pivot
    matrix1 = corr_cat_cat.pivot(index="cat_1", columns="cat_2", values="pearson")
    matrix2 = corr_con_cat.pivot(index="cat", columns="cont", values="corr_ratio")
    matrix3 = corr_con_con.pivot(index="cont_1", columns="cont_2", values="pearson")

    # Continuous / Continuous pairs heatmap
    fig_con_con = go.Figure(
        data=go.Heatmap(
            z=matrix3.values,
            x=matrix3.columns.values,
            y=matrix3.index.values,
            colorscale="RdBu",
            zmin=-1,
            zmax=1,
        )
    ...
    fig_con_cat = go.Figure(
        data=go.Heatmap(
            z=matrix2.values,
            x=matrix2.columns.values,
            y=matrix2.index.values,
            colorscale="RdBu",
            zmin=-1,
            zmax=1,
        )
    ...
    fig_cat_cat = go.Figure(
        data=go.Heatmap(
            z=matrix1.values,
            x=matrix1.columns.values,
            y=matrix1.index.values,
            colorscale="RdBu",
            zmin=-1,
            zmax=1,
        )
