 CREATE TEMPORARY TABLE IF NOT EXISTS team_pitch_temp
 ENGINE = MEMORY
 AS
 SELECT
 tbc.*,
 ...
 change column types in team_pitch_temp table
...
-- create and populate temporary table with team batting counts and related features
 CREATE TEMPORARY TABLE IF NOT EXISTS feature_temp
 AS
 SELECT
 tbc.*,
 b.away_runs,
 ...
 tpt.P_Strikeout,
 tpt.P_Walk,
 tpt.P_startingInning,
 tpt.P_endingInning
 FROM
 team_batting_counts tbc
 JOIN team_pitch_temp tpt
 ON tbc.game_id = tpt.game_id
 AND tbc.team_id = tpt.team_id
 JOIN team_results tr
 ON tr.team_id = tbc.team_id
 AND tr.game_id = tbc.game_id
 JOIN boxscore b
 ON tbc.game_id = b.game_id
 GROUP BY
 team_id,
 game_id;
 ...
 ROUND((rdh.P_IP/rda.P_IP),3) AS IP_Ratio,
 ...
 FROM
 game g
 JOIN rolling_100_days rdh
 ON g.game_id = rdh.game_id AND g.home_team_id = rdh.team_id
 JOIN rolling_100_days rda
 ON g.game_id = rda.game_id AND g.away_team_id = rda.team_id
 JOIN boxscore b
 ON b.game_id = g.game_id;

 CREATE INDEX game_id_index ON game (game_id);
 CREATE INDEX home_team_id_index ON rolling_100_days (home_team_id);
 CREATE INDEX away_team_id_index ON rolling_100_days (away_team_id);
 CREATE INDEX boxscore_game_id_index ON boxscore (game_id);
 CREATE INDEX rdh_game_id_team_id_index ON rolling_100_days (game_id, team_id);
