...
DROP TEMPORARY TABLE IF EXISTS Baseball_common_stats;
CREATE TEMPORARY TABLE IF NOT EXISTS Baseball_common_stats ENGINE=MEMORY AS
SELECT
...
--  CREATE UNIQUE INDEX Baseball_common_stats_idx ON Baseball_common_stats(team_id, game_id);
...
game g JOIN Baseball_common_stats rdh
ON g.game_id = rdh.common_game_id AND g.home_team_id = rdh.common_team_id
JOIN Baseball_common_stats rda ON g.game_id = rda.common_game_id AND g.away_team_id = rda.common_team_id
JOIN boxscore b ON b.game_id = g.game_id;


DROP TABLE IF EXISTS common_and_ratio_features;

--
CREATE TABLE IF NOT EXISTS common_and_ratio_features AS
SELECT * FROM Baseball_ratio_features bf JOIN
Baseball_common_stats bs ON bf.game_id_ratio = bs.common_game_id;

DROP TEMPORARY TABLE IF EXISTS batting_avg_per_batter;

CREATE TEMPORARY TABLE batting_avg_per_batter AS
...
CREATE TABLE IF NOT EXISTS batting_common_features
select * from batting_avg_per_batter bag join common_and_ratio_features crf
on bag.bag_game_id = crf.game_id_ratio ;


DROP TABLE IF EXISTS all_stats_features;
--
CREATE TABLE IF NOT EXISTS all_stats_features
select * from batting_common_features bcf join pitcher_stat ps
on bcf.game_id_ratio = ps.game_id;

-- select * from all_stats_features;